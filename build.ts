...
`,
    ...
    },
    {
      ...
      setup(build) {
        ...
        // Resolve .d.ts imports to actual file paths
        build.onResolve({ filter: /\.d\.ts$/ }, async (args) => {
          const path = await import('path')
          const resolved = path.resolve(path.dirname(args.importer), args.path)
          return { path: resolved, namespace: 'dts-shim' }
        })
        // Load .d.ts files as empty modules (they're type-only)
        build.onLoad({ filter: /.*/, namespace: 'dts-shim' }, () => ({
          contents: 'export {}',
          loader: 'js',
        }))
      },
